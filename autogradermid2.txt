...
function grade() {
 ...
 //			$text = str_ireplace("\x0D", "", $text); 
      ...
      $write = "+ calling newlinekiller(text) just in case the student spams newline characters\n";
      $text = newlineKiller($text);
      $write .= $text . "\n"; 
      autolog($write, $target);
   
      $write = "+ calling colonfixer(text); text: \n"; 
      $write .= $text . "\n"; 
      $write .= "+ trimming the whitespaces:  \n"; 
      $text = trim($text);
      $write .= $text . "\n"; 
      autolog($write, $target); 

      $write = "+ sanitize the text off characters. calling hiddencharkiller()\n";
      $text = hiddencharkiller($text); 
      $write .= $text . "\n"; 
      autolog($write, $target); 
    ...
    if (! $status ) { 
        ...
        foreach($array as $key=>$c) {
				...
				if ($c ===  $arrayofOuts[$key]) {
					...
					//	$feed = "np testcase '". $tests[$key] . "' passed!"; 
					$function = getFunc($tests[$key]);
					//$output = getOut($tests[$key]);
					$feed = "np Called " . $function . ", expected: \"" . $arrayofOuts[$key] . "\", got \"" . $c  ."\"" ;
					$write .= "+ " . $feed . "\n"; autolog($write, $target);   
  ...
  function funcom($text , $tests, $id, $qId, $constat) {
    ...
    if ($constat) {
		...
		foreach($tests as $x) {
			...
			$write = "+ np obtained function " . $function . " with funcom()\n"; autolog($write, $target); 
       ...
       /* grab the function from user answer */
		...
		if ($miss >= $size) {
			...
			$write = "+ the function was found in the user answer\n"; autolog($write, $target); 
                        $feed = "np Expecting function: \"".$function."\", found \"" .$function."\""; 
                        $subpoints = "0"; 
                        update($id, $qId, $feed, $subpoints, $max);  
			return 0; 
		}
 ...
 function newlineKiller($text) {
	/* kill the new line character in the beginning of a string */ 
	/* task A: only grab contents before def */
	
	if($newlines = str_replace(PHP_EOL, "x", $text)) {
		//echo "fucking: " . $newlines . ",\n";
		$end = stripos($text, 'def', 0);
		$newtext = substr_replace($text, "", 0, $end);
